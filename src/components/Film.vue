  ...
  <div class="listpage" id="film">
    <div class="listpage-header" id="film-header">
      ...
      <el-autocomplete
        ...
        popper-class="popper"
        placeholder="搜索"
        @keyup.enter="searchAndRecord"
        @select="searchEvent"
        @change="searchChangeEvent">
        <el-select v-model="searchGroup" size="small" v-slot="prepend"
          :popper-append-to-body="false"
          popper-class="popper"
        ...
        <!--方便触屏-->
        <el-button icon="el-icon-search" @click.stop="searchEvent" v-slot="append" v-if="!searchRunning"/>
        <el-button icon="el-icon-loading" @click.stop="stopSearchEvent" v-slot="append" v-if="searchRunning" title='点击可停止搜索'/>
      </el-autocomplete>
    </div>
    ...
    <div class="listpage-body" id="film-body" infinite-wrapper>
      <div class="show-picture" v-if="setting.view === 'picture' && !showFind">
          ...
          backgroundColor="rgba(0, 0, 0, 0)">
            ...
            <template v-slot:item v-slot:props>
              <div class="card">
                <div class="img">
      ...
      <div class="show-table" v-if="setting.view === 'table' && !showFind">
        <el-table
          ...
          <infinite-loading
            ...
            // eslint-disable-next-line vue/valid-v-slot
            <!-- <div v-slot:no-more>数据量过少时请重复操作一次，以防网站抽风</div> -->
              <div >数据量过少时请重复操作一次，以防网站抽风</div>
          </infinite-loading>
        </el-table>
      </div>
